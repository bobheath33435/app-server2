...
usersRouter.get('/h1bCount', async (req, res) => {
    try{
        logger.info('Processing get');
...
usersRouter.get('/h1bWsCd', async (req, res) => {
    try{
...
usersRouter.get('/user', (req, res) => {
    ...
    log(chalk.bgRed(JSON.stringify(req.body)))
    res.status(203).send("Howdy")
    // const user = new modelMap['user'](req.body)
    // user.save().then(() => {
